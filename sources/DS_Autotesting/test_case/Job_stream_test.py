# -*- coding: utf-8 -*-
from Common import DS_Operation, TestException
...
class Job_stream_test:
    ...
    def job_stream_positive_test(self, ds_id, ds_pwd, driver_sequence):
        ...
        if fail_count != 0 :
            ...
            print("\nWhen the driver job run finished successfully, all the dependent job run finished successfully,"
    ...
    def job_stream_nagative_test(self,ds_id,ds_pwd,driver_sequence):
        ...
        if fail_count != 0:
            print("When the driver job run finished successfully,one or more dependent jobs "
                  "run failed, check the job status report "
                  "for the detail,job stream test failed")
            raise TestException.JobStreamError()
        else:
            print("When the driver job run finished successfully, all the dependent job run finished successfully, "
                  "the job stream test run pass")
